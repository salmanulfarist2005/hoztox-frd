    ...
    cartItems.forEach((item) => {
      if (!seenSKUs.has(item.product.SKU)) {
        ...
        uniqueCartItems.push({
          ...
          ...(item.color ? { color: item.color } : {}), // only add if present
            ...
            cartItems.map((item) => (
              <div className="cart-card" key={item?.id}>
                ...
                <ul className="cart-summary">
                  <li>
                    <span className="cart-key">Quantity:</span>{" "}
                    <span className="cart-value">{item?.quantity}</span>
                  </li>
                  <li>
                    <span className="cart-key">Color:</span>{" "}
                  ...
                  <li>
                    <span className="cart-key">Gross Weight:</span>{" "}
                    <span className="cart-value">{item?.gross_weight} gm</span>
                  </li>
                  <li>
                    <span className="cart-key">Diamond Weight:</span>{" "}
                    <span className="cart-value">
                      {item?.diamond_weight} Ct
                    </span>
                  </li>
                  <li>
                    <span className="cart-key">Color Stones:</span>{" "}
                    <span className="cart-value">{item?.colour_stones} Ct</span>
                  </li>
                  <li>
                    <span className="cart-key">Net Weight:</span>{" "}
                    <span className="cart-value">{item?.net_weight} gm</span>
                  </li>
                </ul>
                <div className="notes-section">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleNoteChange(item?.product?.SKU, e.target.value)
                    }
                    placeholder="Add any specific instructions or notes here"
                  />
                </div>
                <Link to="/cart" className="edit-cart-button">
                  Edit Item
                </Link>
              </div>
            ))
        ...
        {cartItems.length > 0 && (
          <div className="total-weights">
            ...
            <button
              ...
              className="fz-1-banner-btn cart-checkout-btn bg-gold"
            >
              Place Order
            </button>
          </div>
